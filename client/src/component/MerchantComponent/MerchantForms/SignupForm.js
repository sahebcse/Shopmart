...
const SignupForm = () => {
    ...
    const [bankDetails, setBankDetails] = useState({bankName:'', holderName:'', accountNumber:'', ifscCode:''})
    const classes = useStyles()
    const [activeStep, setActiveStep] = useState(0);
        const Form = ()=> {
            switch (activeStep) {
                case 0:
                    return <StoreDetails selectedCSC={selectedCSC} setSelectedCSC={setSelectedCSC}  />
                case 1:
                    return <BankDetails bankDetails={bankDetails} setBankDetails={setBankDetails} />
                case 2:
                    return <Login selectedCSC={selectedCSC} bankDetails={bankDetails} isSignin={isSignin} />
            }
        }
    ...
    return (
        ...
        <main className={classes.layout}>
            ...
            {(activeStep<steps.length) && <button className="bg-blue-500 py-2 px-3 m-3 hover:bg-blue-700 rounded" onClick={handleNextPage}>Next</button>}
            </> : <Login selectedCSC={selectedCSC} bankDetails={bankDetails} isSignin={isSignin} />}
             <div>
