  ...
  const intilizeScoreList = () => {
    ...
    for (var i = 0; i < days; i++) {
      var monthname = new Date(
        `${selectedDate.getFullYear()}-${selectedDate.getMonth() + 1}-${i + 1}`
      )
        .toDateString()
        .slice(4, 10);
    ...
    const fetchScore = async () => {
      const response = await fetch(
        `http://localhost:5000/order/merchant/${user?.result?._id}`,
        {
          method: "POST",
          headers: {
          ...
          body: JSON.stringify({
            startDate: new Date(
              `${selectedDate.getFullYear()}-${selectedDate.getMonth() + 1}-01`
            ).getTime(),
            endDat: new Date(
              `${selectedDate.getFullYear()}-${
                selectedDate.getMonth() + 1
