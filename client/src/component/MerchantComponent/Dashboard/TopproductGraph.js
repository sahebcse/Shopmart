...
const TopproductGraph = () => {
    ...
    const [allMerchantProductsByProfit,setAllMerchantProductsByProfit] = useState([])
    ...
    useEffect(()=>{
        ...
        if(allMerchantProductsByProfit.length>0){
            ...
            for(let i=0;i<Math.max(MAX_TOP_PRODUCTS,allMerchantProductsByProfit.length);i++){
                newMaxList.push(allMerchantProductsByProfit[allMerchantProductsByProfit.length-1-i]?.totalProfit)
    ...
    useEffect(()=>{
        if(allMerchantProductsBySold.length>0){
            ...
            for(let i=0;i<Math.max(MAX_TOP_PRODUCTS,allMerchantProductsBySold.length);i++){
                newMaxList.push(allMerchantProductsBySold[allMerchantProductsBySold.length-1-i]?.totalSold)
  ...
  return (
    <div className="flex flex-wrap justify-around">
        <div className="w-full md:w-3/4 m-2 px-2 py-2">
            <span className="text-lg font-bold">Top Selling Products (By Total Profit)</span>
            <Bar data={replacableGraphData} options={options} />
        </div>
        <div className="w-full md:w-3/4 m-2 px-2 py-2">
            <span className="text-lg font-bold">Bottom Selling Products (By Total Profit)</span>
            <Bar data={replacableGraphData} options={options} />
        </div>
        <div className="w-full md:w-3/4 m-2 px-2 py-2">
            <span className="text-lg font-bold">Bottom Selling Products (By Total Sold)</span>
            <Bar data={replacableGraphData} options={options} />
        </div>
        <div className="w-full md:w-3/4 m-2 px-2 py-2">
            <span className="text-lg font-bold">Bottom Selling Products (By Total Sold) </span>
